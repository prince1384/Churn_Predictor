...
const Dashboard: React.FC<DashboardProps> = ({
	...
	return (
		<div className="flex h-screen bg-gray-900 text-white">
			{showChatbot && (
				<div className={`transition-all duration-300 ease-in-out w-96 bg-gray-800 flex-shrink-0`}>
			...
			<div className="flex-grow p-8 overflow-y-auto">
				<div className="flex justify-between items-center mb-8">
						<h1 className="text-3xl font-bold">Churn Prediction Dashboard</h1>
						<Link to="/report">
								<button className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50">
										Report Generator
								</button>
						</Link>
				</div>
				...
				{!showChatbot && (
						<button onClick={() => setShowChatbot(true)} className="fixed bottom-8 right-8 p-3 rounded-full bg-blue-600 text-white shadow-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 z-50">
				...
				<div className="space-y-8">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
								{metricCards.map((card, index) => (
										<motion.div 
												key={index} 
												className={`bg-gradient-to-br ${card.gradient} backdrop-blur-sm border border-white/10 p-6 rounded-xl shadow-lg relative overflow-hidden`}
												whileHover={{ scale: 1.03, boxShadow: '0px 10px 30px rgba(0, 0, 0, 0.4)' }}
										...
										>
												...
												<div className="flex justify-between items-start relative z-10">
														...
														<div className="flex flex-col items-end">
																...
																{card.indicator && <div className={`flex items-center ${card.isPositive ? 'text-green-400' : 'text-red-400'}`}>{card.indicator}</div>}
														</div>
												</div>
										</motion.div>
								))}
						...
						<div className="mt-8">
								{activeTab === 'overview' && (
										<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
												<motion.div className="bg-gradient-to-br from-gray-800/80 to-gray-900 border border-white/10 p-6 rounded-xl shadow-lg relative overflow-hidden" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
												...
												<motion.div className="bg-gradient-to-br from-gray-800/80 to-gray-900 border border-white/10 p-6 rounded-xl shadow-lg relative overflow-hidden" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
								...
								{activeTab === 'predictions' && (
										<motion.div className="bg-gradient-to-br from-gray-800/80 to-gray-900 border border-white/10 p-6 rounded-xl shadow-lg relative overflow-hidden" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
